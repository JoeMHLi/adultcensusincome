...
##########################################################
# Number of question marks in the adult dataset
colSums(adult == "?")
...
nzv_tbl
# Figures on the near zero variables, descending order of proportion
adult %>% count(capital.gain) %>% mutate(pct = n / sum(n)) %>% arrange(desc(pct))
adult %>% count(capital.loss) %>% mutate(pct = n / sum(n)) %>% arrange(desc(pct))
adult %>% count(native.country) %>% mutate(pct = n / sum(n)) %>% arrange(desc(pct))
...
rm(dl, adult_tmp, nzv, nzv_tbl, adult_r, p_capgain, p_caploss, p_country, test_index)

##########################################################
# Exploratory Data Analysis
##########################################################

# Proportion of anuual income exceeding $50k
...
# Race -----------------------------------
# Race List (descending by number of people)
train_set %>% count(race) %>% arrange(desc(n))
...
# Education -----------------------------------
# Education List (ascending by education level)
train_set %>% group_by(education, education.num) %>% count() %>% arrange(education.num)
...
# Occupation -----------------------------------
# Occupation List (descending by number of people)
train_set %>% count(occupation) %>% arrange(-n)
...
# Workclass -----------------------------------
# Workclass List (descending by number of people)
train_set %>% count(workclass) %>% arrange(desc(n))
...
# Relationship -----------------------------------
# Relationship List (descending by number of people)
train_set %>% count(relationship) %>% arrange(desc(n))
...
# Imputation and Random Forest with Imputation-----------------------------------

# sources:
# https://blog.mitsde.com/data-imputation-techniques-handling-missing-data-in-machine-learning/
...
train_set %>% count(occupation, income) %>% 
  group_by(occupation) %>% 
  mutate(pct = n / sum(n) * 100) %>% 
  filter(occupation == "Exec-managerial" | is.na(occupation)) %>% 
  arrange(desc(n)) %>%
